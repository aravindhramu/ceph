...
int RGWPostObj_REST_S3::read_with_boundary(bufferlist& bl, uint64_t max, bool check_crlf,
                                           bool *reached_boundary, bool *done)
{
  ...
  if (max > in_data.length()) {
    ...
    uint64_t read_len;
    CGI_GetStr(s, bp.c_str(), need_to_read, read_len);
  ...
  *done = false;
  ...
  if (*reached_boundary) {
    ...
    if (left < skip + 2) {
      ...
      int actual;
      CGI_GetStr(s, boundary_bp.c_str(), need, actual);
...
int RGWPostObj_REST_S3::read_line(bufferlist& bl, uint64_t max,
				  bool *reached_boundary, bool *done)
{
  return read_with_boundary(bl, max, true, reached_boundary, done);
}

int RGWPostObj_REST_S3::read_data(bufferlist& bl, uint64_t max,
				  bool *reached_boundary, bool *done)
{
...
int RGWPostObj_REST_S3::read_form_part_header(struct post_form_part *part,
                                              bool *done)
{
  bufferlist bl;
...
bool RGWPostObj_REST_S3::part_str(const string& name, string *val)
{
...
int RGWPostObj_REST_S3::get_params()
{
  string temp_line;
  string param;
  string old_param;
...
int RGWPostObj_REST_S3::verify_permission()
{
  ...
  if (part_str("policy", &encoded_policy_str)) {
    ...
    if (part_str("AWSAccessKeyId", &s3_access_key)) {
      ...
      if (part_str("signature", &signature_str)) {
        ldout(s->cct, 0) << "Attached signature found!" << dendl;
...
int RGWPostObj_REST_S3::get_data(bufferlist& bl)
{
  bool boundary;
  bool done;
...
void RGWPostObj_REST_S3::send_response()
{
  set_req_state_err(s, ret);
...
RGWOp *RGWHandler_REST_S3::get_post_op()
{
  if (s->object) {
    ...
    return new RGWPostObj_REST_S3;
  }

  // check if the bucket name was omitted from the url
  else if (!s->object && !s->bucket_name) {
    string content_type(s->content_type);
    string content_type_head;
    int pos = content_type.find("/");
    if (pos < 0)
      return NULL;

    content_type_head = content_type.substr(0, pos);
    if (strcmp(content_type_head.c_str(), "multipart") == 0)
      return new RGWPostObj_REST_S3;
  }
